...
class Calculator {
  ...
  clear() {
    ...
    this.previousCalculation = ""; //
    this.currentCalculation = "";
    this.isCompleteCompute = false; // Trạng thái đã hoàn thành phép tính hay chưa??
    this.count = 0;
    this.updateDisplay();
  }
  ...
  compute(isCompleteCompute) {
    ...
    if (!this.previousCalculation) {
      this.previousCalculation += `${parseFloat(this.previousOperand)} ${this.operation
        } ${parseFloat(this.currentOperand)}`;
    } else {
  ...
  updateDisplay() {
    ...
    if (this.previousCalculation) {
      this.previousOperandTextElem.innerText = `${this.previousCalculation} ${this.operation ? this.operation : ""
        }`;
    } else {
      if (this.operation) {
        this.previousOperandTextElem.innerText = `${parseFloat(this.previousOperand)} ${this.operation}`;
      } else {
